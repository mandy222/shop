<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="java.util.*,cn.edu.lingnan.dto.GoodsDTO"%>
	...
	function deleteCheck(){
		...
		if(flag == true){
			if(confirm("您确定要删除这些记录吗?"))
				location.href="deleteCheckGoodsInfoServlet?arr="+arr;			
		}else
			alert("您至少选择一条记录，再进行删除");					
	}
	
	function insertCheck(){
		location.href="insertCheckGoodsInfo.jsp";
	}	
</script>
        ...
        <nav class="navbar-default navbar-side" role="navigation">
            <div class="sidebar-collapse">
                <ul class="nav" id="main-menu">
                    <li>
                        <a href="findAllUserInfoServlet"><i class="fa fa-user fa-fw"></i></i> 用户管理</a>
                    </li>

                    <li>
                        <a href="showAllGoodsInfo.jsp" class="active-menu"><i class="fa fa-qrcode"></i> 商品管理</a>
                    </li>
	                        ...
	                        <div class="panel-body">
	                            <div class="table-responsive">
	                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
										...
										<tbody>
											<tr>
												<td><input type="checkbox" name="check" value=<%=gd.getGid() %>></td>
												<td><%=gd.getGid() %></td>
												<td><%=gd.getGname() %></td>
												<td><%=gd.getPrice() %></td>
												<td><%=gd.getCount() %></td>
												<td>  
													<a href="deleteGoodsInfoServlet?gid=<%=gd.getGid() %>">删除</a> 
													<a href="updateGoodsInfo.jsp?gid=<%=gd.getGid() %>">修改</a>
												</td>
											</tr>	
										</tbody>	
										<% 			
									...
									<input type="button" value="添加商品" onClick="insertCheck();"/>
									<pre>  </pre>		
								</div>
		          			</div>
	             		</div>
